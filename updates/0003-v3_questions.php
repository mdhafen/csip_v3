...
Overall [input]% of students where proficient on the [[SELECT category_name FROM category WHERE version = 3 AND course_group = [course_group] AND course_group_order = [course_group_order] -1]] CRT.
<input type=\"submit\" name=\"op\" value=\"Save Answers\">' ),
 ( 3, 16, 1, 2, 8, 0, 'Subgroup(s), with 10 or more members, which had less than [[SELECT [answer_0] FROM answer CROSS JOIN question USING (questionid) WHERE csipid = [csipid] AND categoryid = [categoryid] AND part = 1 AND version = 3 AND order_num = 2]]% proficient?
...
<ol style=\"list-style-type: upper-alpha;\"><li>Userid = usoedd
Password = cognos</li><li>Use the drop-down menus to view CRT results for any district, school, or year.</li><li>Use the drop-down menus to filter by demographic group and click SUBMIT.</li><li>The graphs and tables are drill-down links.</li><li>To see the numbers behind the percents click CRT BY NUMBER AND PERCENT at the bottom of the report.</li><li>CRT BY TEACHER displays class results by teacher Cactus number.</li></ol>
Note: DATA DISPLAY is for educational users only.  DATA DISPLAY will not report individual students results.

Below, enter any data analysis information the PLC found to be important (or as directed by the Principal)
[input]' ),
