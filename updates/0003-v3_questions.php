...
<b>Overall [input]% of students were proficient on the [[SELECT category_name FROM category WHERE version = 3 AND course_group = [course_group] AND course_group_order = [course_group_order] -1]] CRT.</b>
...
Safe Harbor Provision: AYP may also be met by reducing the number of students <u>not proficient</u> by 10% from the previous year.' ),
...
[i_percent [[SELECT answer FROM answer CROSS JOIN question USING (questionid) WHERE csipid = [csipid] AND categoryid = [categoryid] AND part = 1 AND version = 3 AND order_num = 1]] ]% students not proficient
